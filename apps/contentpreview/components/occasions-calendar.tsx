...
import { occasions, occasionsToConfirm, occasionCategories, eventsOnDate, OCCASION_START, OCCASION_END, OCCASION_REVIEWED, type Occasion } from '@/lib/occasions';
...
function OccasionRow({ event }: { event: Occasion }) {
  return <article className="occasion-row">
    ...
    <div className="occasion-description"><div className="occasion-tags"><span>{event.category}</span><span className={event.status === 'Tentative' ? 'occasion-tentative' : ''}>{event.status}</span></div><h3>{event.title}</h3><small>{event.region}</small><p>{event.idea}</p><a href={event.source} target="_blank" rel="noopener noreferrer">Check source <ExternalLink size={12} /></a></div>
  </article>;
}
...
export default function OccasionsCalendar() {
  ...
  const filtered = useMemo(() => occasions.filter(event => (category === 'All occasions' || event.category === category) && `${event.title} ${event.region} ${event.idea}`.toLowerCase().includes(query.toLowerCase().trim())), [query, category]);
  const monthEnd = new Date(Date.UTC(Number(month.slice(0, 4)), Number(month.slice(5, 7)), 0)).toISOString().slice(0, 10);
  const monthly = filtered.filter(event => event.date <= monthEnd && (event.endDate || event.date) >= `${month}-01`);
  ...
  return <section className="occasions-page" aria-labelledby="occasions-title">
    ...
    {view === 'month' && <div className="occasions-month"><div className="occasions-month-bar"><button aria-label="Previous month" disabled={monthIndex === 0} onClick={() => changeMonth(months[monthIndex - 1])}><ChevronLeft size={18} /></button><select aria-label="Choose occasion month" value={month} onChange={e => changeMonth(e.target.value)}>{months.map(item => <option key={item} value={item}>{monthName(item)}</option>)}</select><button aria-label="Next month" disabled={monthIndex === months.length - 1} onClick={() => changeMonth(months[monthIndex + 1])}><ChevronRight size={18} /></button></div>
      ...
      <div className="occasions-days">{Array.from({ length: offset }, (_, i) => <div className="occasion-blank" key={`blank-${i}`} />)}{Array.from({ length: dayCount }, (_, i) => {
        ...
        const events = eventsOnDate(date, filtered);
        const outside = date < OCCASION_START || date > OCCASION_END;
        return <button key={date} disabled={outside} className={`occasion-day ${selected === date ? 'selected' : ''} ${date === today ? 'today' : ''}`} aria-pressed={selected === date} aria-label={`${format(date, { dateStyle: 'full' })}, ${events.length} occasions${outside ? ', outside planning range' : ''}`} onClick={() => setSelected(selected === date ? null : date)}><span className="occasion-day-number">{i + 1}</span><span className="occasion-day-events">{events.slice(0, 2).map(event => <span key={event.id}>{event.title}</span>)}{events.length > 2 && <small>+{events.length - 2} more</small>}</span>{events.length > 0 && <span className="occasion-day-dot" aria-hidden="true" />}</button>;
      })}</div><p className="occasions-calendar-hint">Select a date to explore its occasions and content ideas.</p></div>}
