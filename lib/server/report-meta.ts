...
async function mediaList(
  ...
  let after = context.pageCursor || '';
  for (
    let page = 0;
    page < (context.importMode === 'content' ? 1 : 20);
    page++
  ) {
    const r = await graphGet(
      ...
      encodeURIComponent(context.externalId) +
        ...
        query({
          fields,
          limit: context.importMode === 'content' ? '10' : '50',
          ...(after ? { after } : {}),
          ...(!instagram
            ? {
                since: range.start,
    ...
    if (context.importMode === 'content') {
      if (r.paging?.next && !after)
        throw new Error(
          'INPUT:Meta did not return a continuation cursor. Retry this page.',
        );
      result.nextCursor = r.paging?.next ? after : null;
    }
    if (
      !r.paging?.next ||
      !after ||
    ...
    if (instagram) {
      ...
      range.end < new Date(Date.now() - 86400000).toISOString().slice(0, 10)
        ...
        const raw: any = await collect(
          ...
          (r) =>
            r.data?.[0]?.total_value?.breakdowns?.[0]?.results?.length || 0,
        );
        const values =
        ...
        if (values.length)
          result.tables.push({
            ...
            rows: values.map((r: any) => ({
              [breakdown]: (r.dimension_values || []).join(' Â· '),
  ...
  if (context.importMode !== 'reports') {
    ...
    if (!instagram && media?.length) {
      const formats = await graphBatch(
        ...
        media.map(
          (m: any) =>
            encodeURIComponent(m.id) + '?fields=attachments{media_type,type}',
        ),
      );
      for (let i = 0; i < media.length; i++)
        if (formats[i]?.body?.attachments)
          media[i].attachments = formats[i].body.attachments;
      ...
      result.checks.push({
        ...
        detail:
          returned +
          ' lifetime interaction totals returned.' +
          (failure
            ? ' Check pages_read_user_content in the login configuration and reconnect. ' +
              failure.error
            : ''),
      });
    }
        ...
        ? m.media_product_type === 'STORY' ? 'Story' : m.media_product_type === 'REELS'
          ...
          : m.media_type === 'CAROUSEL_ALBUM'
            ...
            : m.media_type === 'VIDEO'
              ? 'Video'
              : 'Static'
        : String(m.attachments?.data?.[0]?.type || '').includes('reel')
          ? 'Reel'
          : String(m.attachments?.data?.[0]?.media_type || '')
                .toLowerCase()
                .includes('video')
            ? 'Video'
            : String(m.attachments?.data?.[0]?.media_type || '')
