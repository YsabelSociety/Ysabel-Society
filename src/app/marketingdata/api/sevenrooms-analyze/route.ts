...
export async function POST(req:Request){try{
 ...
 let next=fromBatch;const batches=Math.ceil(rows.length/200),nextBatch=Math.min(batches,fromBatch+12);await Promise.all(Array.from({length:4},async()=>{while(next<nextBatch){const batch=next++;await call({op:'stage',id,batch,rows:rows.slice(batch*200,(batch+1)*200)});}}));
