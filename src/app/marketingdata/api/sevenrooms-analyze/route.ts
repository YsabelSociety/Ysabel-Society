...
export async function POST(req:Request){try{
 ...
 const workbook=new ExcelJS.Workbook();await workbook.xlsx.load(Buffer.from(await raw.arrayBuffer()) as any);
 const sheet=workbook.worksheets[0];if(!sheet)throw new Error('No worksheet found.');
 const scalar=(v:any):unknown=>v instanceof Date?v.toISOString():v&&typeof v==='object'?(v.result??v.text??v.richText?.map((x:any)=>x.text).join('')??''):v??'';
 const headersRaw=(sheet.getRow(1).values as any[]).slice(1).map(x=>String(scalar(x)).trim());
