...
export async function GET(req:Request){try{
 ...
 const rows:Record<string,string|number>[]=[];q.set('op','export');
 ...
 const fields=scope==='email'?['email']:scope==='name'?['name','email']:['name','email','phone','birthday','visits','last_visit','preferred_venue'];
 if(format==='copy')return Response.json({emails:rows.map(r=>r.email)},{headers:{'Cache-Control':'private, no-store'}});
 if(format==='xlsx'){const w=new ExcelJS.Workbook();const s=w.addWorksheet('Marketing audience');s.columns=fields.map(key=>({header:key.replaceAll('_',' '),key,width:key==='email'?36:24}));for(const r of rows)s.addRow(Object.fromEntries(fields.map(k=>[k,safe(r[k])])));s.getRow(1).font={bold:true,color:{argb:'FF1D3428'}};s.views=[{state:'frozen',ySplit:1}];return new Response(await w.xlsx.writeBuffer() as BodyInit,{headers:{'Content-Type':'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet','Content-Disposition':'attachment; filename="ysabel-marketing-audience.xlsx"','Cache-Control':'private, no-store'}});}
 const csv='\uFEFF'+[fields,...rows.map(r=>fields.map(k=>safe(r[k])))].map(r=>r.map(s=>'"'+String(s).replaceAll('"','""')+'"').join(',')).join('\r\n');return new Response(csv,{headers:{'Content-Type':'text/csv;charset=utf-8','Content-Disposition':'attachment; filename="ysabel-marketing-audience.csv"','Cache-Control':'private, no-store'}});
