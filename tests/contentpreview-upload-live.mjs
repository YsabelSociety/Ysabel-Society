...
const website = 'https://ysabelsociety.com/contentpreview';
const login = await fetch(website + '/api/auth/login', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ username: 'ysabel', password }) });
assert.equal(login.status, 200);
const cookie = login.headers.getSetCookie().find(value => value.startsWith('ysabel_session='))?.split(';')[0];
assert.ok(cookie, 'Website must establish its saved cookie session');
const sessionResponse = await fetch(website + '/api/auth/session?upload-check=' + Date.now(), { headers: { cookie } });
assert.equal(sessionResponse.status, 200);
const { authenticated, token } = await sessionResponse.json();
assert.equal(authenticated, true);
assert.ok(token, 'Cookie-only website session must provide a verified upload token');
